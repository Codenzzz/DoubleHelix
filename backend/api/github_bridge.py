...
def _gh_get_file_sha(repo: str, path: str, branch: str, token: str) -> str | None:
    ...
    r = requests.get(url, params={"ref": branch}, headers=_gh_headers(token))
...
def _gh_upsert_file(repo: str, path: str, branch: str, message: str, content_str: str, token: str):
    ...
    r = requests.put(url, json=payload, headers=_gh_headers(token))
...
def edit_file(req: EditReq):
    # 1) Always write locally (makes local dev predictable)
    ...
    if req.push:
        ...
        gh = _gh_upsert_file(repo=repo, path=req.path, branch=branch,
                             message=req.message, content_str=req.content, token=token)
        return {"ok": True, "mode": "local+github", "path": str(target),
                "sha256": sha_local, **gh}
    else:
        return {"ok": True, "mode": "local-only", "path": str(target),
                "sha256": sha_local}
